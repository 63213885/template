...
struct SegmentTree {
	...
	T Max(int u, int l, int r) {
		if (tr[u].l >= l && tr[u].r <= r) return tr[u].x;
		int mid = tr[u].l + tr[u].r >> 1;
		pushdown(u);
		T ans = -2e9;
		if(l <= mid) ans = max(ans, Max(u << 1, l, r));
		if (r > mid) ans = max(ans, Max(u << 1 | 1, l, r));
		return ans;
	}
};
...
struct TreeChainPartition : SegmentTree<T> {
	...
	T Max_path(int u, int v) {
		T ans = -2e9;
		while (top[u] != top[v]) {
			if (d[top[u]] < d[top[v]]) swap(u, v);
			ans = max(ans, Max(1, dfn[top[u]], dfn[u]));
			u = f[top[u]];
		}
		if (d[u] > d[v]) swap(u, v);
		ans = max(ans, Max(1, dfn[u], dfn[v]));
		return ans;
	}
	T Max_tree(int u) {
		return Max(1, dfn[u], dfn[u] + sz[u] - 1);
	}
};